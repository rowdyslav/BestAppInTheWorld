...
class User:
    def __init__(self, email, fio) -> None:
        self.email = email
        self.fio = fio
...
class OfficeManager(OfficeWorker):
    def __init__(self, email, fio, office: Office) -> None:
        super().__init__(email, fio)
...
class RestaurantAdmin(User):
    ...
    def add_office(self, user_email, address="Адрес оффиса по умолчанию") -> None:
        OFFICES.insert_one(Office(user_email, address))

    def remove_office(self, office_id) -> None:
        OFFICES.find_one_and_delete({'id': office_id})
