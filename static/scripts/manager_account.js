...
function toggleCartPopup() {
	...
	var htmlCode =
		'<button class="close_btn" onclick="closeCartPopup()"><img src="../static/images/remove.png"></button>' +
	...
	for (var i = 0; i < items.length; i++) {
		...
		htmlCode +=
			...
			"</div>" +
			'<div class="counter">' +
			'<div class="btn" onclick="minusButton(' +
			i +
			')">-</div>' +
			'<div class="count">' +
			item[2] +
			"</div>" +
			'<div class="btn plus" onclick="plusButton(' +
			i +
			')">+</div>' +
			"</div>" +
			'<div class="prices">' +
			...
			item[1] * item[2] +
			"₽</div>" +
			'<div class="remove"><u><button onclick="deleteItem(' +
			i +
			')"><img src="../static/images/delete_icon_red.png"></button></u></div>' +
			"</div>" +
			"</div>";
	}
...
function makeOrder() {
	...
	if (is_delivery) {
		...
		var orderData = {
			is_delivery: is_delivery,
			address: { officeFloor: null, officeNumber: null, placeNumber: null },
			dishes: [],
		};
	}
