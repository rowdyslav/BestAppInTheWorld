...
def get_worker_keyboard() -> ReplyKeyboardMarkup:
    start_rkm = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="/auth"), KeyboardButton(text="/help")]],
                                    resize_keyboard=True)
    return start_rkm


def get_deliverier_keyboard() -> ReplyKeyboardMarkup:
    deliverier_rkm = ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text="/orders"), KeyboardButton(text="/help")]],
                                         resize_keyboard=True)
...
def get_orders_inline(orders: list) -> InlineKeyboardMarkup:
    ...
    for i, order in enumerate(orders):
        buttons[0].append(InlineKeyboardButton(text=f"№{i + 1} {order['status']}", callback_data=f"order_{order['_id']}"))
...
async def wait_auth(message: types.Message):
    if not (message.text and message.from_user) or not SESSION.get(message.from_user.id):
        return
...
async def help(message: types.Message):
    await message.answer(
        HELP_TEXTS.get(type(SESSION[message.from_user.id]), '*/auth* - авторизация'), parse_mode="Markdown",
